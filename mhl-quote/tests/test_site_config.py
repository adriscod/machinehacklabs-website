...
def test_site_json_matches_yaml() -> None:
    ...
    materials = yaml_data.get("materials") or {}
    ...
    assert json_data["meta"]["costs_and_mrr"] == "TODO_REPLACE"
    assert "al_6061" in json_data["materials"]
    assert "ss_304" in json_data["materials"]
    assert json_data["turnaround"]["rush"]["labor_mult"] == 1.5
    assert json_data["tolerance"]["tight"] == 1.25
